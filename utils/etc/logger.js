...
const fs = require("fs");
const Guild = require("../models/mod-log");
 ...
 * @param {String[]} args
 */
module.exports = async (c, message, args) => {
  ...
  `
    ...
    c.on("messageUpdate", function (oldMessage, newMessage) {
      ...
      send_log(
        ...
        "Message UPDATED",
        `
  **Author : ** <@${newMessage.member.user.id}> - *${
  ...
  **THE PERMISSIONS CHANGED PLEASE CHECK!!!**
  OLD PERMISSIONS: ${
    /*perms[String(oldRole.permissions.bitfield)]*/ oldRole.permissions.bitfield
  }
  NEW PERMISSIONS: ${
    /*perms[String(newRole.permissions.bitfield)]*/ newRole.permissions.bitfield
  }
  **Role ID:** \`${newRole.id}\``
        );
      }
...
async function send_log(c, guild, color, title, description, thumb) {
  try {
    ...
    const LogEmbed = new Discord.MessageEmbed()
      ...
      .setFooter(
        guild.name + " Coded By GrimoireDev",

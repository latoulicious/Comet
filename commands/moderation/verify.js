  ...
  run: async (client, message, args) => {
    ...
    if (verifiedrole.managed)
      return message.channel
        .send("**Cannot Add That Role To The User!**")
        .then((m) => m.delete({ timeout: 2000 }));
    if (message.guild.me.roles.highest.comparePositionTo(verifiedrole) <= 0)
      return message.channel
        .send(
    ...
    if (
      ...
      return message.channel
        .send(
          "**Cannot Add Role To This User! - [Higher Than Me In Role Hierachy]**"
        )
        .then((m) => m.delete({ timeout: 2000 }));
